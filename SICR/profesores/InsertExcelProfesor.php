<?php
	...
	if ($_FILES['archivo']['error'] > 0){
		...
		print("<script>window.location.replace('ExcelProfesor.php');</script>");
	}
	else{
		...
		elseif ($opcion == 1) {
			for($i = 2; $i <= $numRows; $i++){
				$Nombre = $objPHPExcel->getActiveSheet()->getCell('A'.$i)->getCalculatedValue();
				$AP_materno = $objPHPExcel->getActiveSheet()->getCell('B'.$i)->getCalculatedValue();
				$AP_paterno = $objPHPExcel->getActiveSheet()->getCell('C'.$i)->getCalculatedValue();
				$titulo = $objPHPExcel->getActiveSheet()->getCell('D'.$i)->getCalculatedValue();
				$nivel = $objPHPExcel->getActiveSheet()->getCell('E'.$i)->getCalculatedValue();

				$cedula= $objPHPExcel->getActiveSheet()->getCell('F'.$i)->getCalculatedValue();
				$tipo= $objPHPExcel->getActiveSheet()->getCell('G'.$i)->getCalculatedValue();
				$clave_presupuestal= $objPHPExcel->getActiveSheet()->getCell('H'.$i)->getCalculatedValue();
				$status= $objPHPExcel->getActiveSheet()->getCell('I'.$i)->getCalculatedValue();
				$curp = $objPHPExcel->getActiveSheet()->getCell('J'.$i)->getCalculatedValue();

				$edo_civil = $objPHPExcel->getActiveSheet()->getCell('K'.$i)->getCalculatedValue();
				$correo_electronico = $objPHPExcel->getActiveSheet()->getCell('L'.$i)->getCalculatedValue();
				$calle = $objPHPExcel->getActiveSheet()->getCell('M'.$i)->getCalculatedValue();
				$colonia = $objPHPExcel->getActiveSheet()->getCell('N'.$i)->getCalculatedValue();
				$municipio = $objPHPExcel->getActiveSheet()->getCell('O'.$i)->getCalculatedValue();

				$estado = $objPHPExcel->getActiveSheet()->getCell('P'.$i)->getCalculatedValue();
				$nombramiento = $objPHPExcel->getActiveSheet()->getCell('Q'.$i)->getCalculatedValue();
				$fecha_de_ingreso = $objPHPExcel->getActiveSheet()->getCell('R'.$i)->getCalculatedValue();
				$departamento = $objPHPExcel->getActiveSheet()->getCell('S'.$i)->getCalculatedValue();
				$telefono = $objPHPExcel->getActiveSheet()->getCell('T'.$i)->getCalculatedValue();
				$celular = $objPHPExcel->getActiveSheet()->getCell('U'.$i)->getCalculatedValue();
				$numero = $objPHPExcel->getActiveSheet()->getCell('V'.$i)->getCalculatedValue();
				$codigo_postal = $objPHPExcel->getActiveSheet()->getCell('W'.$i)->getCalculatedValue();
				$rfc = $objPHPExcel->getActiveSheet()->getCell('X'.$i)->getCalculatedValue();
				
				
				$sql = "INSERT INTO profesor(nombre, ap_materno, ap_paterno, titulo, nivel, cedula, tipo, clave_de_plaza_presupuestal, status, curp, edo_civil, correo_electronico, calle, colonia, municipio, estado, nombramiento, fecha_de_ingreso, departamento, telefo, celular, numero, codigo_postal, rfc_profesor)VALUES ('$Nombre', '$AP_materno', '$AP_paterno', '$titulo', '$nivel', '$cedula', '$tipo', '$clave_presupuestal', '$status', '$curp', '$edo_civil', '$correo_electronico', '$calle', '$colonia', '$municipio', '$estado', '$nombramiento', '$fecha_de_ingreso', '$departamento', '$telefono', '$celular',  '$numero', '$codigo_postal', '$rfc')";

				$result = pg_query($dbconn, $sql) or die('<script language="javascript">alert("Error en el Servidor");</script> <script>window.location.replace("ExcelProfesor.php");</script>');
